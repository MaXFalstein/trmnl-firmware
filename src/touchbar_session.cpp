...
// Tracks first detection of both corners held (portal power-off gesture)
...
static bool check_corners_gesture() {
  ...
  Log_info("Hold edges slider mode: event=%d (HOLD=%d) left=%d right=%d", slider_event, IQS323_GESTURE_HOLD, left,
           right);
  return slider_event == IQS323_GESTURE_HOLD && left && right;
}

static void check_channel_states(void) {
...
static void read_slider_coordinates(void) {
  /* read slider coordinates from memory */
  uint16_t coord = iqs323.sliderCoordinate();

  if (coord != slider_position) {
    slider_position = coord;
  }
}

/* Function to process Slider gesture events */
static void read_gesture_event(void) {
...
bool touchbarHasOtgMessage(void) { return otg_message; }
